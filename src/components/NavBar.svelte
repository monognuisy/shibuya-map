...
<Pane class="w-nav pt-8 px-12 pb-6">
  <div class="flex items-center gap-6">
    ...
    <button
      class="w-28 cursor-pointer rounded-4 border border-amber bg-amber px-0 py-4 text-center
             ...
             hover:text-tx compact:min-h-40 compact:text-base"
    ...
    <div class="flex min-w-0 flex-1 flex-wrap items-baseline gap-6 text-sm text-tx2 compact:text-base">
      {#if leg}
        <span class="font-medium text-tx">{legSummary(leg)}</span>
        <span class="font-mono text-xs text-tx3 compact:text-sm"
          >{Math.round(leg.distance)} m · {formatDuration(leg.seconds)}</span
        >
        {#if leg.arrival}
          <span class="text-tx2">
            → {leg.arrival.name}{#if leg.arrivalDetail}
              <span class="ml-3 rounded-3 border border-amber-dim px-4 text-xs text-amber compact:text-sm"
                >{leg.arrivalDetail}</span
              >{/if}
            <span class="font-mono text-xs text-tx3 compact:text-sm">{levelCode(leg.toLevel)}</span>
          </span>
        {/if}
      {:else}
    ...
    <label class="flex items-center gap-4 text-xs text-tx3 compact:text-sm">
      배속
      <select
        bind:value={app.navSpeed}
        class="cursor-pointer rounded-4 border border-line2 bg-panel2 px-4 py-2 font-mono text-sm
               text-tx2 compact:min-h-40 compact:text-base"
      >
  ...
  <input
    class="mt-6 w-full cursor-pointer accent-amber compact:h-40"
    type="range"
    min="0"
    max={total}
    step="1"
