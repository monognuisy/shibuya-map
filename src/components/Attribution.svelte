...
<div class="relative">
  <button
    class="cursor-pointer rounded-4 border border-line bg-panel/90 px-10 py-4 font-mono text-2xs tracking-8 text-tx3 uppercase hover:border-line2 hover:text-tx2 compact:min-h-40 compact:text-sm"
  ...
  {#if open && meta}
    <Pane class="absolute bottom-28 left-1/2 w-attr-sheet -translate-x-1/2 px-15 py-12 text-left">
      ...
      >
      <h3 class="mb-8 font-mono text-sm tracking-12 text-tx3 uppercase compact:text-base">데이터 출처</h3>
        ...
        {#each meta.sources as s (s.key)}
          <li class="grid gap-2">
            <a
              ...
              class="justify-self-start border-b border-line2 text-sm text-tx no-underline hover:text-amber compact:text-base"
              >{s.title}</a
            >
            <span class="text-xs text-tx3 leading-normal compact:text-sm">{s.license}</span>
            <span class="text-xs text-tx3 leading-normal compact:text-sm">취득 {s.fetchedAt.slice(0, 10)}</span>
          </li>
        {/each}
        <li class="grid gap-2">
          <span class="justify-self-start border-b border-line2 text-sm text-tx compact:text-base"
            >역 구내 레이어 (개찰 · 승강장 · 연락통로)</span
          >
          <span class="text-xs text-tx3 leading-normal compact:text-sm"
      ...
      <div class="mt-12 border-t border-line pt-8 font-mono text-2xs text-tx3 compact:text-sm">
