...
const passages: CuratedPlace[] = [
  {
    ...
    width: 8,
    desc: '2020년 9월 26일 공용 개시. 시부야역과 후쿠라스를 지상 2층 레벨로 연결한다. 이 데크가 열리면서 옛 도큐백화점 도요코점 안 통로와 JR 다마가와개찰이 폐쇄됐다. 선형은 OSM `しぶにしデッキ`/`渋⾕フクラス接続デッキ` 에서 가져왔다.',
...
export const CURATED_LINKS: CuratedLink[] = [
  ...
  // 개찰 밖 — 지상·데크
  { from: 'gate-jr-hachiko', to: 'plaza-hachiko', kind: 'walk' },
  ...
  { from: 'pass-nishi-deck', to: 'plaza-hachiko', kind: 'escalator', note: '2F → 1F' },
  ...
  // 개찰 밖 — 지하
  { from: 'gate-dt-hachiko', to: 'plaza-shibuchika', kind: 'walk', note: 'B2 → B1' },
  { from: 'gate-dt-dogenzaka', to: 'plaza-shibuchika', kind: 'walk', note: 'B2 → B1' },
  { from: 'plaza-shibuchika', to: 'plaza-hachiko', kind: 'stairs', note: 'B1 → 1F' },
