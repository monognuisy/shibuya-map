...
- 경유지 안내: 어디를 지나 어디로 가는지, 승강장 어느 쪽 끝인지까지
- 경로 따라가기: 경로 위를 진행 방향 시점으로 훑어볼 수 있음
...
승강장을 대표점 하나로 두면 「남쪽 끝에서 북쪽 개찰까지 200m」라는 사실이 사라진다.
그래서 중심선이 있는 지점(승강장·연락통로)은 35m 간격 노드 체인으로 만들고, 체인
위를 실제 거리만큼 걷게 한다. JR 야마노테선 승강장은 220m 짜리 체인이 된다.

curated 링크는 **상대 지점에 가장 가까운 체인 노드**에 붙는다. 어느 계단·에스컬레이터를
쓰는지 손으로 지정하지 않아도, 하치코개찰은 북쪽에 신남개찰은 남쪽에 붙고 그 둘은
100m 넘게 떨어진다. 「같은 통로인데 다른 에스컬레이터를 타면 돌아가야 한다」가
경로 비용에 그대로 반영된다.

### 수직 동선은 한 대씩 센다

OSM 에 층 정보가 붙은 계단·에스컬레이터·엘리베이터를 개별 시설로 세운다. 층 구간,
운행 방향(단방향/양방향/시간대별 전환), 휠체어 접근성을 함께 들고 있고, 각 링크는
`v` 필드로 어느 설비인지 되짚을 수 있어서 경로 안내가 「어느 에스컬레이터인지」를
말할 수 있다.
...
경로 안내 아래 「경로 따라가기」를 누르면 카메라가 경로 위로 내려가 진행 방향을
바라본다. 드래그하면 진행 방향 기준으로 좌우를 둘러보고, 휠로 거리를 조절한다.
...
| `Esc` | 따라가기 끄기 |
| 구간 목록 클릭 | 그 구간으로 이동 |
| 드래그 / 휠 | 진행 방향 기준으로 둘러보기 / 거리 조절 |

경로 선 굵기는 왼쪽 아래 「경로 굵기」에서 조절한다(반지름 0.6–6 m).
...
| `ex` | 층 간격 배율 (1–8) |
| `rw` | 경로 선 굵기(반지름, m) |
| `nav` | 경로 따라가기를 이 진행 거리에서 시작 (읽기 전용) |
| `play` | 1이면 열자마자 따라가기 재생 (읽기 전용) |
